import { OrderSchedule, UrgencyInfo, UrgencyLevel } from '@/types';
...
export function getUrgencyInfo(orders: OrderSchedule[]): UrgencyInfo {
  ...
  const daysLeft = Math.floor((orderDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
...
export function groupOrdersByDate(orders: OrderSchedule[]): Record<string, OrderSchedule[]> {
...
export function getHeatmapIntensity(orders: OrderSchedule[] | undefined): number | 'overdue' {
  if (!orders || orders.length === 0) return 0;
  const hasOverdue = orders.some(o => !o.actualOrderDate && new Date(o.orderDate) < new Date());
